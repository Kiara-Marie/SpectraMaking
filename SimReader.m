...
pathPrefix = "C:\Users\Kiara\Documents\glw\CleanBifurcation\Results\";
pathSuffix = "KP_den_0p01\UniformDataConcat_Den_0p01_shells_20_t_max_800.csv";
betweenOffset = 2;
megaFigure = figure();
...
for currD = 1:length(rangeOfDen)
    ...
    for n = 1:6:((6*(maxN-minN))+1)
        m = m+1 ;
        time_full = real(OutputMatrix(:,n));
        Ryd_t_full=real(OutputMatrix(:,n+1));
    ...
    for n = 1:6:((6*(maxN-minN))+1)
        m = m + 1 ;
        time_full = real(OutputMatrix(:,n));
        electrons_full=real(OutputMatrix(:,n+3));
